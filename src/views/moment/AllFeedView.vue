<template>
    <div class="all-feed h-full flex flex-col">
        <LoadingMoreContainer class="mb-0!" fill-parent :handle-load="handleLoad" v-model:is-more="isMore"
            v-model:is-loading="isLoading" v-model:is-error="isError"
            :show-end-text="items.length > 0">
            <template #content>
                <EmptyState v-if="!isLoading && !isError && items.length === 0" text="还没有动态，快来发布第一条吧 ~" />
                <div v-else class="all-feed__list space-y-4 max-w-2xl mx-auto py-4">
                    <MomentCard v-for="item in items" :key="item.dynIdStr" :item="item"
                        :status="statusOf(item.dynIdStr)" :show-more-actions="true" @click="openDetail"
                        @src-click="openDetail" @avatar-click="openUserSpace" @thumb="handleThumb(item)"
                        @report="handleReport(item)" @remove="handleRemove(item)" />
                </div>
            </template>
        </LoadingMoreContainer>
        <!-- 换一换（B 站 rcmd 风格：右侧固定，垂直"换一换"文字，刷新推荐流） -->
        <div v-if="items.length > 0"
            class="all-feed__refresh fixed right-4 top-1/2 -translate-y-1/2 z-20 cursor-pointer select-none rounded-lg bg-bg-secondary border border-border-primary text-text-secondary hover:text-primary hover:bg-bg-overlay transition-colors duration-150 px-2 py-3 shadow-md text-sm font-medium opacity-80 hover:opacity-100 [writing-mode:vertical-rl] tracking-[0.25em]"
            :class="{ 'is-loading pointer-events-none opacity-60': isLoading }"
            role="button" tabindex="0" aria-label="换一换推荐内容"
            @click="handleRefresh" @keydown.enter="handleRefresh">
            <span class="all-feed__refresh-text">换一换</span>
        </div>
...
import { fetchAllFeed, fetchInteractionStatus, InteractionBizTypeEnum, ReportBizTypeEnum, thumbMoment } from '@/api/notify/moment-api'
...
const handleLoad = async () => {
    ...
    try {
        ...
        const res = await fetchAllFeed({
            ps: 20,
            last_showlist: showlist.length
                ? showlist.slice(-SHOWLIST_LIMIT).join(',')
                : undefined,
            uniq_id: getUniqId(),
        })
