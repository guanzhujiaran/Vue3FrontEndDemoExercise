<template>
  <div class="avatar-audit-list h-full flex flex-col">
    ...
    <LoadingWrap :loading="loading" class="avatar-audit-list__content flex-1 min-h-0 overflow-y-auto">
      <EmptyState v-if="!loading && items.length === 0" text="暂无待审核头像" />
      <div v-else class="avatar-audit-list__table-wrap">
        <el-table
          :data="items"
          class="avatar-audit-list__table w-full"
          size="default"
          stripe
          ...
          <el-table-column label="申请者" width="160">
            <template #default="{ row }">
              <div class="flex items-center gap-2">
                <el-avatar :size="28" :src="row.oldAvatar || BiliImg.face.noface" />
                <span class="truncate text-sm">{{ row.authorName || row.mid || '未知' }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="旧头像" width="90">
            <template #default="{ row }">
              <el-avatar :size="48" :src="row.oldAvatar || BiliImg.face.noface" shape="square" />
            </template>
          </el-table-column>
          <el-table-column label="新头像" width="90">
            <template #default="{ row }">
              <el-avatar :size="48" :src="row.newAvatar || BiliImg.face.noface" shape="square" />
            </template>
          </el-table-column>
        ...
        <PaginationBar
          v-if="total > pageSize"
          class="avatar-audit-list__pagination mt-4"
          :total="total"
          :page-size="pageSize"
          :current-page="page"
