...
const lot_upload_selected_drop_down_props = computed<LotUploadAreaProps>(() => {
  if (
    active_lot_data_name.value &&
    (!lot_upload_command.value ||
      lot_upload_command.value?.title !== lot_upload_drop_down_props.value.choose_one.title)
  ) {
    switch (active_lot_data_name.value) {
      case 'GetOfficialLottery':
      case 'GetReserveLottery':
      case 'GetChargeLottery':
        return lot_upload_drop_down_props.value.upload_dynamic_lottery
    }
  }
  return lot_upload_command.value
    ? lot_upload_command.value
    : lot_upload_drop_down_props.value.choose_one
})
const handle_lot_upload_submit = async () => {
  ...
  lot_upload_submit_props.value.submit.result_data = Array.from(
    lot_upload_submit_props.value.submit.req_data.map(() => {
      return {
        code: -1,
        data: '无数据',
        msg: '无数据'
  ...
  await Promise.all(
    lot_upload_submit_props.value.submit.req_data.map(async (da) => {
      return lotteryDataBaseApi
        .handle_add_dynamic_lottery_data({ dynamic_id_or_url: da })
        .then((result) => {
          lot_upload_submit_props.value.submit.result_data[
            lot_upload_submit_props.value.submit.req_data.indexOf(da)
          ] = result
        })
    })
  )
