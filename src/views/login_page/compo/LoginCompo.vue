 ...
 * @LastEditTime: 2024-05-31 15:58:06
...
const handleLoginBtn = async () => {
  if (user_name.value && pwd.value) {
    toastRef.value!.info(`请输入账号和密码！`);
    return
  }
  let resp = await userApi.Login(user_name.value, pwd.value);
  if (resp.code) {
...
const handleRegBtn = async () => {
  if (reg_user_name.value && reg_pwd.value) {
    toastRef.value!.info(`请输入账号和密码！`);
    return
  }
  let resp = await userApi.Reg(reg_user_name.value, reg_pwd.value);
  if (resp.code) {
...
<template>
  ...
  <div class="main__right">
    ...
    <div class="tab-sms" v-if="login_info.checked === 'reg'">
      <div class="tab__form">
        <div class="form__item">
          <input autocomplete="off" maxlength="32" oninput="value=value.replace(/[^\d]/g, '')" placeholder="请输入需要注册的账号名"
            v-model="reg_user_name" />
        </div>
        <div class="form__separator-line"></div>
        <div class="form__item">
          <input autocomplete="off" placeholder="请输入注册密码" maxlength="32" oninput="value=value.replace(/[^\d]/g, '')"
            v-model="reg_pwd" />
        </div>
      </div>
