    ...
    >
      <template #content>
        <EmptyState
          ...
          :text="t('message.emptyLike')"
        />
        <ul v-else class="like-list__items space-y-3 py-4">
          <EventItemCard
            v-for="item in items"
            :key="item.id"
            :item="item"
...
async function handleLoad() {
  ...
  try {
    ...
    // 首屏：拉取列表后自动把「当前时刻之前」的点赞全部置为已读（取代原「全部已读」按钮）
    if (isFirst) await markReadBeforeNow()
  } catch (e) {
    console.error('加载收到的赞列表失败:', e)
    isError.value = true
  } finally {
