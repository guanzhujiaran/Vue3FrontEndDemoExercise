  ...
  <FlexContainer class="bili-lottery-data-panel gap-6 pb-8">
    ...
    >
      <div class="flex flex-col gap-6 rounded-lg bg-bg p-5 sm:p-6">
        <div class="flex flex-1 space-y-4">
          <BiliPageHeader title="话题抽奖数据" description="B 站话题相关的抽奖数据" tag-text="话题抽奖" tag-type="info" />

          <div class="grid grid-cols-2 gap-3 flex-1 pl-20 sm:pl-10 md:pl-15">
            <el-statistic
              class="stat-card"
              title="筛选结果"
              :value="topic_lot_data_props.lot_data?.total ?? 0"
              :value-style="statValueStyle"
            />
            <el-statistic
              class="stat-card"
              title="当前页码"
              :value="topic_lot_data_props.lot_page || 1"
              :value-style="statValueStyle"
            />
          </div>
        </div>
    ...
    >
      ...
      <BiliPaginationDataView
        ...
        :total="topic_lot_data_props.lot_data?.total ?? 0"
        :page_size="page_size"
      ...
      >
        ...
        <template #contents>
          ...
          <BiliOfficialLotteryTable
            v-else
            :data="topic_lot_data_props.lot_data?.items ?? []"
            storage-key="topic-lottery-table:fixed-columns"
          />
        </template>
      </BiliPaginationDataView>
    </section>
...
onUnmounted(() => {
  topic_lot_data_props.value.lot_data = { items: [], total: 0 }
  topic_lot_data_props.value.loading = true
  topic_lot_data_props.value.error = false
})
