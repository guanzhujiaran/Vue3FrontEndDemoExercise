...
<template>
  <div class="user-lottery-result-wrapper place-items-center">
    ...
    <BiliEmpty v-else-if="!isError && (!getFilteredPrizeItems || getFilteredPrizeItems.length === 0)">
    </BiliEmpty>
    <BiliError v-else-if="isError" @click-retry="handle_lottery_result"></BiliError>
    <div v-else
      class="user-lottery-result grid grid-cols-1 md:grid-cols-2 gap-6 text-[var(--el-color-white)] text-base mb-8 justify-center w-full max-w-6xl px-4 sm:px-6"
      v-if="getFilteredPrizeItems && getFilteredPrizeItems.length > 0">
      <div v-for="(prize, index) in getFilteredPrizeItems" :key="index" class="prize-item-row flex w-full flex-col">
        <div :class="[
          ...
          <div
            class="-mt-8 ml-5 border-title relative z-[2] text-center w-fit px-3 py-1 rounded-full bg-bg border border-border-light/50">
            <span :class="{
          ...
          <div class="prize-info-wrapper flex flex-col md:flex-row gap-4 p-[0.1rem] relative z-[1]">
            ...
            <div :class="[
              ...
              <p
                :class="['prize-rank font-medium', { 'text-[var(--el-color-danger)]': getPrizeLevel(prize) === '一等奖', 'text-[var(--el-color-primary)]': getPrizeLevel(prize) === '二等奖', 'text-[var(--el-color-warning)]': getPrizeLevel(prize) === '三等奖' }]">
                奖项: {{ getPrizeLevel(prize) }}</p>
              <p class="text-text-secondary text-sm">开奖时间: {{ new Date(prize.lottery_time * 1000).toLocaleString() }}
              </p>
              <p class="jump-url cursor-pointer text-primary hover:underline text-sm mt-1" v-if="
                prize.business_type !== 10 // 非预约抽奖
              " @click="gotoOpusDynamic(prize.business_id)">
                动态ID：{{ prize.business_id }}
              </p>
              <p class="jump-url cursor-pointer text-primary hover:underline text-sm mt-1" v-else
      ...
      <el-pagination hide-on-single-page
        ...
        size="small" :total="data.total" :page-size="props.limit" v-model:current-page="currentPage"
        layout="prev, pager, next,total" :background="false" v-if="data && data.total > 0"></el-pagination>
    </div>
  </div>

  <el-tour v-model="biliRankUserAtariDetailTip" @close="useTourTipStore().isBiliRankUserAtariDetailActive = false">
    <el-tour-step target=".prize-link a" title="中奖图片"
      description="中奖奖品的图片，点击跳转抽奖链接（注意，非动态链接，是h5的链接，https://www.bilibili.com/h5/lottery/result。）"></el-tour-step>
    <el-tour-step target=".prize-info" title="中奖信息"
      description="包含奖品名称，奖品等级，开奖时间，（动态|预约）ID信息。点击动态ID可跳转对应动态，点击预约ID可跳转抽奖详情页。"></el-tour-step>
  </el-tour>
</template>
