<template>
  <div class="action-hover" :class="{ 'compact-mode': compact }">
    <div class="action-header bg-yellow-500/10 border-b border-yellow-500/20 px-3 py-2 flex items-center gap-2 rounded-t-md" :class="compact ? '!py-1 !px-2' : ''">
      <span class="text-lg">👆</span>
      <span class="font-semibold text-yellow-600 text-sm" :class="compact ? '!text-xs' : ''">悬停元素</span>
    </div>
    ...
    <div class="action-body p-3 space-y-3 bg-white dark:bg-gray-800 rounded-b-md border border-t-0 border-gray-200 dark:border-gray-700" :class="compact ? '!p-2 !space-y-2' : ''">
      ...
      <div v-if="!compact">
        <label class="text-xs text-[var(--el-text-color-placeholder)] mb-1 block font-mono">duration (悬停时长 ms)</label>
        <el-input-number v-model="params.duration" :min="0" controls-position="right" class="w-full" size="small" />
      </div>
